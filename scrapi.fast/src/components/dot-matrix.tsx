...
export const DotMatrix: React.FC<PixelBlastProps> = ({
  variant = "diamond",
  pixelSize = 3,
  color,
  className,
  ...
  const prevConfigRef = useRef<Partial<PixelBlastProps>>({})

  const [resolvedTheme, setResolvedTheme] = useState<string>('light')

  useEffect(() => {
    // Detect theme from CSS variable or media query
    const checkTheme = () => {
  ...
  const themeColor =
    color || (resolvedTheme === "dark" ? "hsl(var(--foreground) / 0.6)" : "hsl(var(--foreground) / 0.4)")

  useEffect(() => {
    const container = containerRef.current
    if (!container) return
    speedRef.current = speed
    const needsReinitKeys: Array<keyof PixelBlastProps> = ["antialias"]
    const cfg: PixelBlastProps = { antialias }
    ...
    if (mustReinit) {
      ...
      const initialResolved = resolveCssColor(themeColor, container)
      ...
      const applyResize = () => {
        ...
        if (canvas.width === targetW * pixelRatio && canvas.height === targetH * pixelRatio) return
      ...
      const animate = (currentTime: number) => {
        ...
        const colorStr = threeRef.current?.colorInput ?? themeColor
      ...
      threeRef.current = {
        ...
        colorInput: themeColor,
      }
    } else {
      const t = threeRef.current!
      t.uniforms.uShapeType.value = SHAPE_MAP[variant] ?? 0
      t.uniforms.uPixelSize.value = pixelSize * t.renderer.getPixelRatio()
      const resolvedNow = resolveCssColor(themeColor, container)
      t.uniforms.uColor.value.setStyle(resolvedNow)
      t.lastResolvedColor = resolvedNow
      t.colorInput = themeColor
  ...
  return (
    <div
      aria-label="DotMatrix interactive background"
      className={`-z-10 absolute inset-0 overflow-hidden opacity-95 ${
        className ?? ""
      }`}
      ref={containerRef}
      style={{
    ...
    >
      ...
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_1200px_400px_at_center,var(--background)_0%,transparent_100%)]"
        style={{ transform: "translateZ(0)" }}
      />

      {/* Bottom Gradient */}
      <div
        className="pointer-events-none absolute right-0 bottom-0 left-0 h-[300px] bg-gradient-to-t from-background to-transparent"
        style={{ transform: "translateZ(0)" }}
      />

      {/* Top Gradient */}
      <div
        className="pointer-events-none absolute top-0 right-0 left-0 h-[200px] bg-gradient-to-b from-background to-transparent"
        style={{ transform: "translateZ(0)" }}
      />
    </div>
  )
